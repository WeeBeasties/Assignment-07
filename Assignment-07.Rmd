...
read_csv("./output/deaths_by_state.csv") %>%
  ...
  scale_fill_brewer(type = "seq", palette = 18, name = "Total Deaths")
```
...
```{r danseMacabre, fig.cap = "**Figure 3:** Population Density in 2010", fig.width = 10}
read_csv("./raw_data/pop_density_2010.csv") %>%
  rename(value = population_density_2010) %>%
  as.data.frame() %>%
  state_choropleth(num_colors = 5) +
  scale_fill_brewer(type = "seq", palette = 5, name = "People/sq mi")
```
...
dat <- read_csv("./output/normalized_deaths_by_state.csv") %>%
  rename(region = State) %>%
...
* The large proportion of states with differences between -14 and 24 percent suggests this line of thinking isn't totally flawed.